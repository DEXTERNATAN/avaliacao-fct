...
export class AbrangenciaComponent implements OnInit, AfterViewInit {
  ...
  public abrangenciaCarregada = true;
  ...
  deleteAbrangencia(abrangencia){
    if (confirm('Tem certeza que quer APAGAR a Abrangência #' +
        abrangencia.id_abrangencia + ' (' + abrangencia.atributo + ' '
        + abrangencia.classificacao + '-' + abrangencia.nome + ')?')) {
      let index = this.Abrangencia.indexOf(abrangencia);
