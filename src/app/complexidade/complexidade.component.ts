...
export class ComplexidadeComponent implements OnInit {
  ...
  public complexidadeCarregada = true;
  ...
  deleteComplexidade(complexidade) {
    ...
    if (confirm('Tem certeza que quer APAGAR a Complexidade #' + complexidade.id_complexidade + " (" + complexidade.atributo + ' ' +
      complexidade.classificacao + '-' + complexidade.nome + ')?')) {
      let index = this.Complexidade.indexOf(complexidade);
